...
#define ACS_MAX_VECTOR_LEN \
    (256 * 7) /* Max Possible non operable (Exclude) chanspecs in a radio is 256*/
...
#define NUM_RADIOS_RESULTS_FOR_RNR 2
...
typedef struct {
    ...
    bool radio_presence; // True for ECO mode Active radio, false for ECO mode power down sleeping
                         // radio
    bool radar_detected;
...
typedef int (*app_get_ap_assoc_dev_diag_res3_t)(int ap_index,
    wifi_associated_dev3_t *assoc_dev_array, unsigned int *output_array_size);

typedef int (*app_get_neighbor_ap2_t)(int radio_index, wifi_neighbor_ap2_t *neighbor_results,
    unsigned int *output_array_size);

typedef int (*app_get_radio_channel_stats_t)(int radio_index,
    wifi_channelStats_t *channel_stats_array, int *array_size);
...
typedef int (*platform_set_radio_params_t)(wifi_radio_index_t index,
    wifi_radio_operationParam_t *operationParam);
typedef int (*platform_set_radio_pre_init_t)(wifi_radio_index_t index,
    wifi_radio_operationParam_t *operationParam);
...
typedef int (*platform_get_chanspec_list_t)(unsigned int radioIndex,
    wifi_channelBandwidth_t bandwidth, const wifi_channels_list_t *channels, char *buff);
...
typedef int (*platform_get_radio_phytemperature_t)(wifi_radio_index_t index,
    wifi_radioTemperature_t *radioPhyTemperature);
typedef int (
    *platform_set_dfs_t)(wifi_radio_index_t index, wifi_radio_operationParam_t *operationParam);
...
int wifi_hal_parse_rrm_beacon_rep(wifi_interface_info_t *interface, char *buff, size_t len,
    struct rrm_measurement_beacon_report *meas_rep);
...
typedef enum client_state { ACTIVE = 1, INACTIVE, MAX_STATE } client_state_t;
...
INT wifi_hal_sendDataFrame(int vap_id, unsigned char *dmac, unsigned char *data_buff, int data_len,
    BOOL insert_llc, int protocal, int priority);
...
int wifi_rrm_send_beacon_resp(unsigned int ap_index, wifi_neighbor_ap2_t *bss,
    unsigned int num_ssid, unsigned int token, unsigned int num_count);
...
int handle_public_action_frame(INT ap_index, mac_address_t sta_mac,
    wifi_publicActionFrameHdr_t *ppublic_hdr, UINT len);
...
int nl80211_dfs_cac_started(wifi_interface_info_t *interface, int freq, int ht_enabled,
    int sec_channel_offset, int bandwidth, int bw, int cf1, int cf2);
...
int update_hostap_radio_param(wifi_radio_info_t *radio,
    const wifi_radio_operationParam_t *newParam);
...
int wifi_send_eapol(void *priv, const u8 *addr, const u8 *data, size_t data_len, int encrypt,
...
enum offload_mode {
    ...
    PROBEREQ_OFFLOAD_ON,
    /* turn of wildcard SSID offload    | only necessary frames + wildcards SSID's are forwared to
       user space */
    PROBEREQ_OFFLOAD_WILDCARD_SSID_OFF,
    /* offload OFF                      | all frames are forwarded to user space (may degrade
       performance in a busy environment) */
...
int wifi_set_key(const char *ifname, void *priv, enum wpa_alg alg, const u8 *addr, int key_idx,
...
int wifi_sta_set_flags(void *priv, const u8 *addr, unsigned int total_flags, unsigned int flags_or,
    unsigned int flags_and);
int wifi_sta_disassoc(void *priv, const u8 *own_addr, const u8 *addr, u16 reason_code);
int wifi_sta_assoc(void *priv, const u8 *own_addr, const u8 *addr, int reassoc, u16 status_code,
    const u8 *ie, size_t len);
int wifi_set_ap_wps_ie(void *priv, const struct wpabuf *beacon, const struct wpabuf *proberesp,
...
int wifi_hal_send_mgmt_frame(int apIndex, mac_address_t sta, const u8 *data, size_t data_len,
...
INT wifi_hal_steering_clientDisconnect(UINT steeringgroupIndex, INT apIndex,
    mac_address_t client_mac, wifi_disconnectType_t type, UINT reason);
...
// wifi_halstats
#define wifi_hal_stats_dbg_print(format, ...) \
    wifi_hal_stats_print(WIFI_HAL_STATS_LOG_LVL_DEBUG, format, ##__VA_ARGS__)
#define wifi_hal_stats_info_print(format, ...) \
    wifi_hal_stats_print(WIFI_HAL_STATS_LOG_LVL_INFO, format, ##__VA_ARGS__)
#define wifi_hal_stats_error_print(format, ...) \
    wifi_hal_stats_print(WIFI_HAL_STATS_LOG_LVL_ERROR, format, ##__VA_ARGS__)
...
extern int platform_set_radio(wifi_radio_index_t index,
    wifi_radio_operationParam_t *operationParam);
...
extern int platform_set_radio_pre_init(wifi_radio_index_t index,
    wifi_radio_operationParam_t *operationParam);
...
extern int platform_get_chanspec_list(unsigned int radioIndex, wifi_channelBandwidth_t bandwidth,
    const wifi_channels_list_t *channels, char *buff);
...
extern int platform_get_radio_phytemperature(wifi_radio_index_t index,
    wifi_radioTemperature_t *radioPhyTemperature);
...
extern int platform_get_sta_measurements(void *priv, const u8 *sta_addr,
...
void platform_config_eht_chanspec(wifi_radio_index_t index,
    wifi_radio_operationParam_t *operationParam);
...
void platform_set_chanspec(wifi_radio_index_t index, wifi_radio_operationParam_t *operationParam,
    bool b_check_radio);
#endif
#endif
